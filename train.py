...
def validate_epoch(
    ...
    with torch.no_grad():
        for inputs, targets, instance_ids in tqdm(
...
def main():
    ...
    print("\nCreating datasets...")
    print(
        f"Signal: {config['duration']}s @ {config['sampling_rate']}Hz = "
        f"{int(config['duration'] * config['sampling_rate'])} samples"
    )
    print(f"Frequencies: {config['frequencies']}")
    print(
        f"Total training samples: {int(config['duration'] * config['sampling_rate'])} × {len(config['frequencies'])} = "
    ...
    samples_per_instance = int(config["sampling_rate"] * config["duration"]) * len(
        config["frequencies"]
    )
