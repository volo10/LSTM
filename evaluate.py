...
def evaluate_model(
    model: nn.Module, dataloader: DataLoader, device: str, num_frequencies: int
) -> Dict[str, float]:
    """
    ...
    with torch.no_grad():
        for freq_idx in range(num_frequencies):
            ...
            for t in range(num_samples):
                ...
                input_vec = (
                    torch.cat(
                        [torch.tensor([signal_value], dtype=torch.float32), one_hot]
                    )
                    .unsqueeze(0)
                    .to(device)
                )
...
def main():
    ...
    train_metrics = evaluate_model(
        model, train_loader, device, len(config["frequencies"])
    )
    ...
    test_metrics = evaluate_model(
        model, test_loader, device, len(config["frequencies"])
    )
    ...
    reconstruction_path = os.path.join(
        config["results_dir"], "reconstructed_signals.npz"
    )
    np.savez(
        reconstruction_path,
        mixed_signal=mixed,
