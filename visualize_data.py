...
def plot_datasets():
    ...
    ax1.plot(time_full, train_mixed, 'b-', alpha=0.6, linewidth=0.5, label='Training (noise seed=1)')
    ax1.plot(time_full, test_mixed, 'r-', alpha=0.6, linewidth=0.5, label='Test (noise seed=2)')
    ...
    ax2.plot(time_window_arr, train_mixed[:num_samples_window], 'b-', alpha=0.7, linewidth=1, label='Training')
    ax2.plot(time_window_arr, test_mixed[:num_samples_window], 'r-', alpha=0.7, linewidth=1, label='Test')
    ...
    for i, freq in enumerate(frequencies):
        ax3.plot(time_window_arr, clean_components[i, :num_samples_window], 
    ...
    ax4.plot(time_window_arr, train_noise[:num_samples_window], 'b-', alpha=0.7, 
            linewidth=0.5, label='Training noise')
    ax4.plot(time_window_arr, test_noise[:num_samples_window], 'r-', alpha=0.7, 
            linewidth=0.5, label='Test noise')
    ax4.set_title('Noise (Different between Train/Test)', fontsize=12, fontweight='bold')
    ...
    for freq_idx, freq in enumerate(frequencies):
        ...
        ax.plot(time_samples, inputs, 'b-', alpha=0.7, label='Input (mixed)', linewidth=1)
        ax.plot(time_samples, targets, 'r-', alpha=0.7, label='Target (clean)', linewidth=2)
        ax.set_title(f'Frequency {freq} Hz - First {samples_to_show} samples', fontweight='bold')
    ...
    print(f"✓ Total training samples: {len(train_dataset):,} (10,000 time steps × 4 frequencies)")
    print(f"✓ Total test samples: {len(test_dataset):,} (10,000 time steps × 4 frequencies)")
