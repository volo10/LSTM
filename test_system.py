...
from data_generator import FrequencyExtractionDataset, create_datasets
...
def model():
    ...
    model = FrequencyExtractorLSTM(
        input_size=5, hidden_size=32, num_layers=1, dropout=0.0, output_size=1
    )
    return model
...
def test_state_continuity():
    ...
    if mean_diff > 0.001:
        print(
            "✓ State continuity is working correctly (outputs differ with/without state)"
        )
    else:
