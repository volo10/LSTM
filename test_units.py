...
class TestSinusoidGeneration:
    ...
    def test_generate_sinusoid_shape(self):
        ...
        assert signal.shape == (1000,), "Signal should have 1000 samples for 1s at 1000Hz"
    ...
    def test_generate_sinusoid_frequency(self):
        ...
        assert abs(peak_freq - frequency) < 0.5, f"Peak frequency {peak_freq} should be close to {frequency}"
    ...
    def test_generate_sinusoid_amplitude(self):
        ...
        assert abs(np.max(signal) - amplitude) < 0.01, f"Max value should be ~{amplitude}"
        assert abs(np.min(signal) + amplitude) < 0.01, f"Min value should be ~{-amplitude}"
    ...
    def test_generate_sinusoid_phase(self):
        ...
        assert np.allclose(signal_0, -signal_pi, atol=1e-10), "Phase shift of π should invert signal"
    ...
    def test_generate_sinusoid_formula(self):
        ...
        assert np.allclose(signal, expected), "Signal should match formula A*sin(2πft + φ)"
...
class TestNoisySignalsGeneration:
    ...
    def test_generate_noisy_signals_shape(self):
        ...
        assert mixed_signals.shape == (num_instances, expected_samples), \
            f"Mixed signals shape should be ({num_instances}, {expected_samples})"
        
        assert clean_components.shape == (num_instances, len(frequencies), expected_samples), \
    ...
    def test_generate_noisy_signals_noise(self):
        ...
        assert not np.allclose(mixed_clean, mixed_noisy), "Noisy signal should differ from clean"
        
        # Difference should be on the order of noise_std
        noise = mixed_noisy - mixed_clean
        assert abs(np.std(noise[0]) - 0.5) < 0.1, "Noise std should be close to specified value"
    ...
    def test_generate_noisy_signals_mixing(self):
        ...
        assert np.allclose(mixed_signals[0], manual_mix, atol=1e-10), \
    ...
    def test_generate_noisy_signals_reproducibility(self):
        ...
        assert np.allclose(mixed1, mixed2), "Same seed should produce same mixed signals"
        assert np.allclose(clean1, clean2), "Same seed should produce same clean components"
...
class TestFrequencyExtractionDataset:
    ...
    def sample_dataset(self):
        ...
        return FrequencyExtractionDataset(mixed_signals, clean_components, [1.0, 3.0, 5.0])
    ...
    def test_dataset_length(self, sample_dataset):
        ...
        assert len(sample_dataset) == 60, "Dataset length should be instances * samples * frequencies"
    ...
    def test_dataset_getitem_shape(self, sample_dataset):
        ...
        assert input_vec.shape == (4,), "Input should be 4-dimensional (1 signal + 3 one-hot)"
    ...
    def test_dataset_getitem_target(self, sample_dataset):
        ...
        assert torch.isclose(target, expected_target), "Target should match clean component value"
    ...
    def test_dataset_all_frequencies_covered(self, sample_dataset):
        ...
        assert np.sum([oh[0] for oh in one_hot_vectors]) == 1, "Frequency 0 should appear once"
        assert np.sum([oh[1] for oh in one_hot_vectors]) == 1, "Frequency 1 should appear once"
        assert np.sum([oh[2] for oh in one_hot_vectors]) == 1, "Frequency 2 should appear once"
...
class TestCreateDatasets:
    ...
    def test_create_datasets_return_types(self):
        ...
        assert isinstance(train_dataset, FrequencyExtractionDataset), \
            "Should return FrequencyExtractionDataset"
        assert isinstance(test_dataset, FrequencyExtractionDataset), \
            "Should return FrequencyExtractionDataset"
    ...
    def test_create_datasets_same_signal(self):
        ...
        assert torch.allclose(train_dataset.clean_components, test_dataset.clean_components), \
    ...
    def test_create_datasets_different_noise(self):
        ...
        assert not torch.allclose(train_dataset.mixed_signals, test_dataset.mixed_signals), \
    ...
    def test_create_datasets_size(self):
        ...
        assert len(train_dataset) == expected_length, \
            f"Train dataset should have {expected_length} samples"
        assert len(test_dataset) == expected_length, \
...
class TestFrequencyExtractorLSTM:
    ...
    def test_model_reset_hidden_state(self):
        ...
        # Should be all zeros
        assert torch.allclose(h, torch.zeros_like(h)), "Initial hidden state should be zeros"
        assert torch.allclose(c, torch.zeros_like(c)), "Initial cell state should be zeros"
    ...
    def test_model_state_persistence(self):
        ...
        assert not torch.allclose(state_after_first[0], state_after_second[0]), \
    ...
    def test_model_state_reset_in_forward(self):
        ...
        assert torch.allclose(out1, out1_repeat, atol=1e-6), \
...
class TestIntegration:
    ...
    def test_end_to_end_training_step(self):
        ...
        model = FrequencyExtractorLSTM(input_size=input_size, hidden_size=16, num_layers=1)
    ...
    def test_dataloader_compatibility(self):
        ...
        for batch_size in [1, 4]:
            for shuffle in [True, False]:
                ...
                assert inputs.shape[1] == expected_input_dim, f"Input dimension should be {expected_input_dim}"
...
class TestEdgeCases:
    ...
    def test_single_frequency(self):
        ...
        assert len(train_dataset) == 10, "Should have 10 samples (1 instance * 10 time * 1 freq)"
