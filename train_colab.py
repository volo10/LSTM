...
# ==================== DATA GENERATOR ====================
def generate_single_sinusoid(frequency: float, amplitude: float, phase: float,
...
def generate_noisy_signals(num_instances: int, frequencies: List[float],
                          sampling_rate: int, duration: float,
                          noise_std: float, seed: Optional[int] = None):
    if seed is not None:
        np.random.seed(seed)
    num_samples = int(sampling_rate * duration)
...
def create_datasets(num_train_instances=1, num_test_instances=1,
                   frequencies=[1.0, 3.0, 5.0, 7.0], sampling_rate=1000,
                   duration=10.0, noise_std=0.1, train_seed=1, test_seed=2,
                   signal_seed=42):
    _, clean_components = generate_noisy_signals(1, frequencies, sampling_rate,
                                                duration, 0.0, signal_seed)
    ...
    for epoch in range(num_epochs):
        ...
        if test_loss < best_test_loss:
            ...
            torch.save({'epoch': epoch + 1, 'model_state_dict': model.state_dict(),
                       ...
                       'train_loss': train_loss, 'test_loss': test_loss}, 'best_model.pth')
